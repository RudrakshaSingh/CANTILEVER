      ...
      try {
        ...
        if (data.status === "error") {
          if (data.code === "maximumResultsReached") {
            ...
            if (append) {
              ...
              throw new Error(
                "API results limit reached. Please try a more specific search."
              );
            }
          } else {
            // Other API errors
        ...
        if (append) {
          ...
          setNewsArticles((prevArticles) => {
            const existingUrls = new Set(
              prevArticles.map((article) => article.url)
            );
        ...
        if (error.response && error.response.data) {
          ...
          if (
            ...
            errorData.code === "maximumResultsReached"
          ) {
            setHasMoreArticles(false);
            if (append) {
              // Don't show error toast for limit reached when appending
              ...
              setError(
                "API results limit reached. Please try a more specific search or upgrade your plan."
              );
            }
            return;
          }
          ...
          <div className="flex items-center justify-between mb-8">
            ...
            {totalResults > 0 && (
              <div className="text-sm text-gray-500">
                Showing {newsArticles.length} of {totalResults.toLocaleString()}{" "}
                articles
              </div>
            )}
          </div>
                ...
                <div className="text-center py-12">
                  <div className="bg-gradient-to-r from-green-50 to-blue-50 border border-green-200 rounded-xl p-8 max-w-md mx-auto">
                    ...
                    <p className="text-gray-600 text-sm">
                      You've viewed all {newsArticles.length} available articles
                      for "{id}".
                    </p>
                    <button
                      onClick={() => fetchNews(1, false)}

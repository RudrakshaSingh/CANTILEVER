...
const EditProfileModal = ({ isOpen, onClose, user }) => {
  ...
  const { loading: isUpdating, error: reduxError } = useSelector((state) => state.user);
  ...
  const validateForm = () => {
    ...
    formData.languages.forEach((lang, index) => {
      if (!lang.language.trim()) {
        ...
        newErrors[`language_${index}`] = 'Language name must be 30 characters or less';
      }
    });
    ...
    formData.futureDestinations.forEach((dest, index) => {
      if (!dest.destination.trim()) {
        ...
        newErrors[`destination_${index}`] = 'Destination name must be 100 characters or less';
      }

      // Validate coordinates if provided
      if ((dest.lat && !dest.lng) || (!dest.lat && dest.lng)) {
        newErrors[`coordinates_${index}`] = 'Both latitude and longitude are required';
      }
      ...
      if (dest.plannedDate) {
        ...
        if (plannedDate < today) {
          newErrors[`plannedDate_${index}`] = 'Planned date cannot be in the past';
        }
      }
    });
    ...
    Object.entries(formData.socialLinks).forEach(([platform, url]) => {
      if (url && url.trim()) {
        try {
          ...
          newErrors[`socialLink_${platform}`] = `Please enter a valid ${platform} URL`;
        }
      }
    });
  ...
  const handleRemoveProfilePicture = () => {
    ...
    if (formData.profilePicturePreview && formData.profilePicturePreview.startsWith('blob:')) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const result = await dispatch(updateProfile({ updates: processedData }));
      ...
      if (updateProfile.fulfilled.match(result)) {
        ...
        if (error?.isAuthError) {
          // Auth errors are already handled in the thunk
          return;
        }
        ...
        // Don't show toast here as it's already shown in the thunk
        // Just scroll to top to show any form errors
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div>
                  ...
                  <input
                    ...
                    className={`w-full px-4 py-3 bg-white/80 backdrop-blur-sm border-2 rounded-xl focus:ring-4 focus:ring-purple-500/20 focus:border-purple-500 transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed ${
                      errors.fullName ? 'border-red-400 bg-red-50/50' : 'border-gray-200 hover:border-purple-300'
                    }`}
                ...
                <div>
                  ...
                  <input
                    ...
                    disabled={isLoading}
                    className={`w-full px-4 py-3 bg-white/80 backdrop-blur-sm border-2 rounded-xl focus:ring-4 focus:ring-purple-500/20 focus:border-purple-500 transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed ${
                      errors.mobile ? 'border-red-400 bg-red-50/50' : 'border-gray-200 hover:border-purple-300'
                    }`}
                ...
                <div>
                  ...
                  <input
                    ...
                    className={`w-full px-4 py-3 bg-white/80 backdrop-blur-sm border-2 rounded-xl focus:ring-4 focus:ring-purple-500/20 focus:border-purple-500 hover:border-purple-300 transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed ${
                      errors.dateOfBirth ? 'border-red-400 bg-red-50/50' : 'border-gray-200'
                    }`}
                  />
                  {errors.dateOfBirth && (
                    <p className="text-red-500 text-sm mt-2 flex items-center">
              ...
              <div className="mt-6">
                ...
                <textarea
                  ...
                  className={`w-full px-4 py-3 bg-white/80 backdrop-blur-sm border-2 rounded-xl focus:ring-4 focus:ring-purple-500/20 focus:border-purple-500 transition-all duration-300 resize-none disabled:opacity-50 disabled:cursor-not-allowed ${
                    errors.bio ? 'border-red-400 bg-red-50/50' : 'border-gray-200 hover:border-purple-300'
                  }`}
            ...
            <div className="bg-white/70 backdrop-blur-sm rounded-2xl p-6 shadow-lg border border-gray-200/50">
              ...
              <div className="space-y-4">
                {formData.languages.map((lang, index) => (
                  <div key={index} className="flex items-center space-x-4 p-4 bg-gradient-to-r from-green-50 to-emerald-50 border border-green-200 rounded-xl">
                    <div className="flex-1">
                      <input
                        ...
                        disabled={isLoading}
                        className={`w-full px-4 py-2 bg-white/80 backdrop-blur-sm border-2 rounded-lg focus:ring-4 focus:ring-green-500/20 focus:border-green-500 transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed ${
                          errors[`language_${index}`] ? 'border-red-400' : 'border-gray-200'
                        }`}
                        placeholder="Language name"
                        maxLength={30}
                      />
                      {errors[`language_${index}`] && (
                        <p className="text-red-500 text-sm mt-1">{errors[`language_${index}`]}</p>
                      )}
                    </div>
                    <div className="w-40">
                      <select
                  ...
                  <div key={platform}>
                    ...
                    <input
                      ...
                      className={`w-full px-4 py-3 bg-white/80 backdrop-blur-sm border-2 rounded-xl focus:ring-4 focus:ring-blue-500/20 focus:border-blue-500 hover:border-blue-300 transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed ${
                        errors[`socialLink_${platform}`] ? 'border-red-400 bg-red-50/50' : 'border-gray-200'
                      }`}
            ...
            <div className="bg-white/70 backdrop-blur-sm rounded-2xl p-6 shadow-lg border border-gray-200/50">
              ...
              <div className="space-y-6">
                {formData.futureDestinations.map((dest, index) => (
                  <div key={index} className="p-6 bg-gradient-to-r from-purple-50 to-indigo-50 border border-purple-200 rounded-xl">
                    <div className="flex items-center justify-between mb-4">
                      <h4 className="font-bold text-gray-900 text-lg">Destination {index + 1}</h4>
                      <button
                        type="button"
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                      <div className="lg:col-span-2">
                        <input
                          ...
                          disabled={isLoading}
                          className={`w-full px-4 py-3 bg-white/80 backdrop-blur-sm border-2 rounded-xl focus:ring-4 focus:ring-purple-500/20 focus:border-purple-500 transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed ${
                            errors[`destination_${index}`] ? 'border-red-400' : 'border-gray-200'
                          }`}
                          placeholder="Destination name"
                          maxLength={100}
                        />
                        {errors[`destination_${index}`] && (
                          <p className="text-red-500 text-sm mt-2">{errors[`destination_${index}`]}</p>
                        )}
                      </div>
                      ...
                      <div>
                        <input
                          ...
                          onChange={(e) => updateDestination(index, 'plannedDate', e.target.value)}
                          disabled={isLoading}
                          className={`w-full px-4 py-3 bg-white/80 backdrop-blur-sm border-2 rounded-xl focus:ring-4 focus:ring-purple-500/20 focus:border-purple-500 transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed ${
                            errors[`plannedDate_${index}`] ? 'border-red-400' : 'border-gray-200'
                          }`}
                          placeholder="Planned date"
                        />
                        {errors[`plannedDate_${index}`] && (
                          <p className="text-red-500 text-sm mt-2">{errors[`plannedDate_${index}`]}</p>
                        )}
                      </div>
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <div>
                          <input
                            ...
                            disabled={isLoading}
                            className={`w-full px-4 py-3 bg-white/80 backdrop-blur-sm border-2 rounded-xl focus:ring-4 focus:ring-purple-500/20 focus:border-purple-500 transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed ${
                              errors[`lat_${index}`] ? 'border-red-400' : 'border-gray-200'
                            }`}
                          ...
                          />
                          {errors[`lat_${index}`] && (
                            <p className="text-red-500 text-sm mt-1">{errors[`lat_${index}`]}</p>
                          )}
                        </div>
                        <div>
                          <input
                            ...
                            disabled={isLoading}
                            className={`w-full px-4 py-3 bg-white/80 backdrop-blur-sm border-2 rounded-xl focus:ring-4 focus:ring-purple-500/20 focus:border-purple-500 transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed ${
                              errors[`lng_${index}`] ? 'border-red-400' : 'border-gray-200'
                            }`}
                          ...
                          />
                          {errors[`lng_${index}`] && (
                            <p className="text-red-500 text-sm mt-1">{errors[`lng_${index}`]}</p>
                          )}
                        </div>
                      </div>
                      {errors[`coordinates_${index}`] && (
                        <p className="text-red-500 text-sm lg:col-span-2">{errors[`coordinates_${index}`]}</p>
                      )}
                    </div>
                  </div>
                ))}
                ...
                {formData.futureDestinations.length === 0 && (
                  <div className="text-center py-8 text-gray-500">
                    ...
                    <p className="text-sm">Click "Add Destination" to plan your next adventure</p>
                  </div>
                )}
              </div>
            </div>
